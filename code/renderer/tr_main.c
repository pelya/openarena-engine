...
void R_DebugPolygon( int color, int numPoints, float *points ) {
	...
	// I'm too lazy to implement debugging stuff
	Com_Printf("Error: R_DebugPolygon() not implemented on GLES\n");
#else
	int		i;
...
#undef CAPABILITY
	...
	for( texUnit = 0; texUnit < maxTexUnits; texUnit++ ) {
		glActiveTexture(GL_TEXTURE0 + texUnit);
		glGetIntegerv(GL_TEXTURE_BINDING_2D, &activeTexId);
		glGetFloatv(GL_CURRENT_TEXTURE_COORDS, texcoords);
		Com_Printf("Texture unit %d enabled %d texID %d texcoord %f %f %f %f\n",
			texUnit, glIsEnabled(GL_TEXTURE_2D), activeTexId, texcoords[0], texcoords[1], texcoords[2], texcoords[3]);
	}
